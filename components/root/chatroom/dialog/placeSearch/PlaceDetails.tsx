...
export default function PlaceDetails({ placeDetails }: {placeDetails: PlaceDetails}) {
  const [hoursDisplay, setHoursDisplay] = useState<boolean>(false);
  const googleMapsUrl = `https://www.google.com/maps?q=${placeDetails.geocodes.main.latitude},${placeDetails.geocodes.main.longitude}`;
  ...
  return (
    <div className='p-2 pt-0 flex flex-col gap-4'>
      <div className='flex flex-col gap-2'>
        ...
        >
          <p className={`font-semibold ${placeDetails.hours.open_now ? 'text-green-500' : 'text-red-500'}`}>
            {placeDetails.hours.open_now ? 'Open' : 'Closed'}
          </p>
          <p>⋅</p>
          <p>{placeDetails.hours.display}</p>
          <div
          ...
          hoursDisplay && (
            <div className='flex flex-col gap-2'>
              <table className="w-full border-collapse text-xs">
                <tbody>
                  {placeDetails.hours.regular.map((day) => {
      ...
      <div className='flex flex-col gap-2'>
        ...
        <div className='flex flex-col gap-2'>
          {
            Object.entries(placeDetails.social_media).map(([key, value]) => value ? (
              <a 
                key={key}
      ...
      <div className='flex flex-col gap-2'>
        ...
        <div className="px-2 py-1 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm">{placeDetails.tel}</p>
            <button 
              onClick={() => handleCopy(placeDetails.tel)}
          ...
          {/* Email */}
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm">{placeDetails.email}</p>
            <button 
              onClick={() => handleCopy(placeDetails.email)}
              className="relative p-1 rounded-md hover:bg-gray-200 transition"
            >
              <Image src={'/dialog/copy-svgrepo-com.svg'} alt='copy' width={16} height={16} />
            </button>
